...
namespace ProjectK.Day21 // 21일차 피해·수배 이름 공간
{
    ...
    public sealed class MapWantedGuardAgent : MonoBehaviour // 자유 추격·수색하는 E-01~E-03 수배 경비
    {
        private const float regularVisionDistance = 65f; // E-01 수배 확장 시야 거리
        private const float eliteVisionDistance = 85f; // E-02 수배 확장 시야 거리
        private const float heavyVisionDistance = 16f; // E-03 기획 시야 거리
        private const float regularVisionAngle = 160f; // E-01 수배 확장 시야각
        private const float eliteVisionAngle = 180f; // E-02 수배 확장 시야각
        private const float heavyVisionAngle = 85f; // E-03 기획 시야각
        ...
        private MapWantedUnitKind unitKind = MapWantedUnitKind.Regular; // 현재 수배 병력 종류
        ...
        public MapWantedUnitKind UnitKind => unitKind; // E-01~E-03 병력 종류 조회
        public bool IsElite => unitKind == MapWantedUnitKind.Elite; // E-02 여부 조회
        public bool IsHeavy => unitKind == MapWantedUnitKind.Heavy; // E-03 여부 조회
        ...
        public void Initialize(MapWantedResponseManager owner, bool isElite) // 기존 E-01·E-02 풀 호환 초기화
        {
            Initialize(owner, isElite ? MapWantedUnitKind.Elite : MapWantedUnitKind.Regular); // bool 등급을 병력 종류로 변환
        }

        public void Initialize(MapWantedResponseManager owner, MapWantedUnitKind kind) // E-01~E-03 공통 초기화
        {
            manager = owner; // 증원 관리자 저장
            wanted = owner != null ? owner.Wanted : null; // 수배 관리자 저장
            unitKind = kind == MapWantedUnitKind.SurveillanceDrone ? MapWantedUnitKind.Regular : kind; // 지상 경비에 드론 종류가 들어오는 상태 방지

            if (unitKind == MapWantedUnitKind.Heavy) // E-03 중장갑 확인
            {
                moveSpeed = 3.8f; // 기획 추격 속도 적용
                attackDistance = 2.9f; // 긴 중장갑 무기 공격 거리 적용
            }
            else if (unitKind == MapWantedUnitKind.Elite) // E-02 정예 확인
            {
                moveSpeed = 6.4f; // 정예 추격 속도 설정
                attackDistance = 2.6f; // 정예 공격 거리 설정
            }
            else // E-01 일반 처리
            {
                moveSpeed = 5.4f; // 일반 추격 속도 설정
                attackDistance = 2.3f; // 일반 공격 거리 설정
            }
        }
        ...
        public void ActivateAt(int nodeIndex) // 풀 경비를 보도 시작점에 배치
        {
            ...
            if (actor != null) // 기존 적 체력 관리자 확인
            {
                ...
                actor.Configure(HealthForKind(), PostureForKind(), unitKind != MapWantedUnitKind.Heavy); // 병력별 체력·자세·암살 여부 설정
            }

            if (sensor != null) // 탐지 센서 확인
            {
                sensor.enabled = true; // 사망 시 꺼졌던 센서 복구
                sensor.Configure(wanted.Player.transform, VisionDistanceForKind(), VisionAngleForKind(), HearingForKind(), ~0); // 병력별 시야·청각 적용
            }
            ...
            if (melee != null) // 근접 전투 확인
            {
                ...
                melee.Configure(sword, BasicDamageForKind(), StrongDamageForKind()); // 병력별 기본·강공격 피해 연결
            }
        ...
        private void Update() // 직접 발견·공유 추적·자유 수색·전투 갱신
        {
            ...
            bool wideVisible = HasWideLineOfSight(playerDistance); // 병력별 시야 거리·각도 확인
            bool visible = sensorVisible || wideVisible; // 기존 센서와 확장 감지 통합
            ...
            if (visible) // 한 경비가 플레이어를 직접 찾은 상태
            {
                ...
                MoveFreely(wanted.Player.transform.position, moveSpeed); // 플레이어 직접 추격
            ...
            if (wanted.HasSharedTarget) // 다른 경비가 최근 발견한 정확한 위치 공유 여부 확인
            {
                ...
                MoveFreely(wanted.SharedTargetPosition, moveSpeed * 0.96f); // 공유 위치로 직접 이동
                ApplyGravity(); // CharacterController 지면 유지
                return; // 공유 추적 처리 종료
            }
            ...
            {
                hasSearchDestination = false; // 각자 새 자유 수색 위치 선택 초기화
                wasSharedPursuit = false; // 분산 수색 상태 전환
            }

            SearchAroundLastKnown(); // 마지막 위치 주변 자유 수색
            ApplyGravity(); // CharacterController 지면 유지
        }

        private bool HasWideLineOfSight(float playerDistance) // 병력별 거리·각도와 근거리 인지
        {
            ...
            float visionDistance = VisionDistanceForKind(); // 병력별 최대 시야 거리 선택
            ...
            if (playerDistance > proximityDetectionDistance && planar.sqrMagnitude > 0.001f) // 근거리 전방향 밖에서만 시야각 검사
            {
                float angle = Vector3.Angle(transform.forward, planar.normalized); // 경비 전방과 플레이어 각도 계산
                float limit = VisionAngleForKind() * 0.5f; // 실제 좌우 반각 계산
                if (angle > limit) // 시야각 바깥 확인
                {
                    return false; // 시야 실패
                }
            }
        ...
        private bool HasClearSightIgnoringDynamicActors(Vector3 start, Vector3 end) // 경비·시민·차량 때문에 서로 시야를 가리지 않도록 검사
        {
            ...
            foreach (RaycastHit hit in hits) // 시야 장애물 후보 순회
            {
                ...
                {
                    continue; // 목표 플레이어는 장애물 제외
                }

                if (IsSightIgnorableDynamic(collider)) // 다른 생활 개체·수배 경비 확인
                {
                    continue; // 동적 군중은 시야 완전 차단에서 제외
                }

                return false; // 건물·벽·고정 구조물에 시야 막힘
            }
        ...
        private static bool IsSightIgnorableDynamic(Collider collider) // 시야에서 무시할 동적 도시 개체 판정
        {
            ...
            {
                return true; // 동료 경비 시야 제외
            }

            if (collider.GetComponentInParent<MapCitizenAgent>() != null) // 시민 확인
            {
                return true; // 군중 시야 제외
            }

            return collider.GetComponentInParent<MapTrafficVehicle>() != null; // 이동 차량 시야 제외
        }
        ...
        private void SearchAroundLastKnown() // 공유 위치가 끝난 뒤 주변 자유 수색
        {
            ...
            if (distance > 1.5f) // 수색 지점 미도착 확인
            {
                MoveFreely(searchDestination, moveSpeed * 0.80f); // 자유 수색 지점 이동
                return; // 이동 중 회전 수색 생략
            }

            RotateSearchPattern(); // 제자리 주변 방향 수색
            if (float.IsPositiveInfinity(nextSearchChange)) // 이번 지점 첫 도착 확인
            {
                float dwell = DwellForKind(); // 병력별 수색 대기 시간 선택
                float variance = Mathf.Abs(GetInstanceID() % 7) * 0.08f; // 경비 동시 이동 방지
        ...
        {
            searchDestination = manager.GetSearchDestination(this, wanted.LastKnownPosition, searchStep); // 자유 수색 위치 조회
            searchStep++; // 다음 수색 순번 증가
            hasSearchDestination = true; // 현재 수색 목적지 활성화
            nextSearchChange = float.PositiveInfinity; // 목적지 도착 전 재선택 금지
        }

        private void RotateSearchPattern() // 수색 지점에서 넓은 시야 회전
        {
            float directionSign = GetInstanceID() % 2 == 0 ? 1f : -1f; // 절반 시계·절반 반시계 회전
            searchLookYaw += directionSign * SearchTurnSpeedForKind() * Time.deltaTime; // 병력별 수색 회전 속도 적용
            Vector3 direction = Quaternion.Euler(0f, searchLookYaw, 0f) * Vector3.forward; // 현재 수색 방향 계산
            RotateTowards(direction); // 경비 몸과 시야 방향 회전
        }

        private void MoveFreely(Vector3 destination, float speed) // 보행 노드 없이 도로와 보도를 직접 이동
        {
            Vector3 delta = destination - transform.position; // 목적지 방향 계산
            delta.y = 0f; // 평면 자유 이동만 사용
            if (delta.sqrMagnitude <= 0.20f) // 목적지 근접 확인
            {
                return; // 이동 중단
            }

            Vector3 desired = delta.normalized; // 가장 빠른 직접 방향 계산
            Vector3 direction = FindFreeDirection(desired); // 고정 구조물 간단 우회
            controller.Move(direction * Mathf.Max(0f, speed) * Time.deltaTime); // 직접 이동 적용
            RotateTowards(direction); // 실제 이동 방향 회전
        }

        private Vector3 FindFreeDirection(Vector3 desired) // 앞이 막히면 좌우로 간단히 우회
        {
            if (IsMoveDirectionOpen(desired)) // 정면 이동 가능 확인
            {
                return desired; // 직접 방향 사용
            }

            float sign = GetInstanceID() % 2 == 0 ? 1f : -1f; // 개체별 우회 우선 방향 분산
            ...
            foreach (float raw in angles) // 우회 후보 순회
            {
                ...
                {
                    return candidate.normalized; // 첫 통과 방향 사용
                }
            }

            return desired; // 완전 차단 시 CharacterController 슬라이딩 사용
        }

        private bool IsMoveDirectionOpen(Vector3 direction) // 자유 이동 앞쪽 고정 장애물 검사
        {
            Vector3 origin = transform.position + Vector3.up * 0.85f; // 허리 높이 검사 원점
            RaycastHit[] hits = Physics.SphereCastAll(origin, movementProbeRadius, direction.normalized, movementProbeDistance, ~0, QueryTriggerInteraction.Ignore); // 전방 장애물 조회
            foreach (RaycastHit hit in hits) // 장애물 후보 순회
            {
                ...
                if (IsSightIgnorableDynamic(collider)) // 경비·시민·차량 확인
                {
                    continue; // 동적 개체를 고정 벽처럼 보지 않음
                }

                if (wanted != null && wanted.Player != null) // 플레이어 참조 확인
                {
                    Transform hitTransform = collider.transform; // 충돌체 Transform 조회
                    if (hitTransform == wanted.Player.transform || hitTransform.IsChildOf(wanted.Player.transform)) // 목표 플레이어 확인
                    {
                        continue; // 추격 대상 장애물 제외
                    }
                }

                return false; // 고정 구조물 차단 방향
            }

            return true; // 자유 이동 가능 방향
        }
        ...
        private void RotateTowards(Vector3 direction) // 경비 진행 방향 회전
        {
            ...
            float turnSpeed = unitKind == MapWantedUnitKind.Heavy ? 6f : 11f; // E-03 느린 방향 전환 적용
            transform.rotation = Quaternion.Slerp(transform.rotation, rotation, turnSpeed * Time.deltaTime); // 추격 회전 적용
        }
        ...
        private float HealthForKind() // 병력별 최대 체력
        {
            if (unitKind == MapWantedUnitKind.Heavy) return 180f; // E-03 체력
            if (unitKind == MapWantedUnitKind.Elite) return 160f; // E-02 수배 정예 체력
            return 100f; // E-01 체력
        }

        private float PostureForKind() // 병력별 최대 자세
        {
            if (unitKind == MapWantedUnitKind.Heavy) return 160f; // E-03 자세
            if (unitKind == MapWantedUnitKind.Elite) return 140f; // E-02 수배 정예 자세
            return 100f; // E-01 자세
        }

        private float VisionDistanceForKind() // 병력별 시야 거리
        {
            if (unitKind == MapWantedUnitKind.Heavy) return heavyVisionDistance; // E-03 기획 시야 거리
            if (unitKind == MapWantedUnitKind.Elite) return eliteVisionDistance; // E-02 수배 시야 거리
            return regularVisionDistance; // E-01 수배 시야 거리
        }

        private float VisionAngleForKind() // 병력별 시야각
        {
            if (unitKind == MapWantedUnitKind.Heavy) return heavyVisionAngle; // E-03 기획 시야각
            if (unitKind == MapWantedUnitKind.Elite) return eliteVisionAngle; // E-02 수배 시야각
            return regularVisionAngle; // E-01 수배 시야각
        }

        private float HearingForKind() // 병력별 청각 반경
        {
            if (unitKind == MapWantedUnitKind.Heavy) return 14f; // E-03 기획 청각 반경
            if (unitKind == MapWantedUnitKind.Elite) return 38f; // E-02 수배 청각 반경
            return 32f; // E-01 수배 청각 반경
        }

        private float BasicDamageForKind() // 병력별 기본 공격 피해
        {
            if (unitKind == MapWantedUnitKind.Heavy) return 30f; // E-03 기획 기본 공격 피해
            if (unitKind == MapWantedUnitKind.Elite) return 28f; // E-02 수배 정예 기본 피해
            return 18f; // E-01 기본 피해
        }

        private float StrongDamageForKind() // 병력별 강공격 피해
        {
            if (unitKind == MapWantedUnitKind.Heavy) return 48f; // E-03 기획 강공격 피해
            if (unitKind == MapWantedUnitKind.Elite) return 42f; // E-02 수배 정예 강공격 피해
            return 30f; // E-01 강공격 피해
        }

        private float DwellForKind() // 병력별 수색 지점 대기
        {
            if (unitKind == MapWantedUnitKind.Heavy) return 1.8f; // E-03 느린 주변 확인
            if (unitKind == MapWantedUnitKind.Elite) return 1.1f; // E-02 빠른 수색
            return 1.5f; // E-01 기본 수색
        }

        private float SearchTurnSpeedForKind() // 병력별 수색 회전 속도
        {
            if (unitKind == MapWantedUnitKind.Heavy) return 75f; // E-03 느린 회전
            if (unitKind == MapWantedUnitKind.Elite) return 130f; // E-02 빠른 회전
            return 105f; // E-01 기본 회전
        }
